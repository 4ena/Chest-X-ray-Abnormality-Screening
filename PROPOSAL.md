...
**Solution:** "We built Pneumanosis — a triage co-pilot. Upload an X-ray, get flagged abnormalities in seconds, with visual heatmaps showing exactly where to look. Patients are automatically ranked by clinical severity tiers — Tier 1 life-threatening cases surface immediately, so the critical ones never wait."
...
Sent to PACS (hospital image storage)
        ↓
  ┌─────────────────────────────┐
  │  Pneumanosis intercepts      │  ← Our insertion point
  │  Runs DenseNet-121          │
  │  Generates Grad-CAM         │
